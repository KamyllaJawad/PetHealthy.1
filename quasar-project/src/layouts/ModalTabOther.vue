<template>
  <div>
    <q-form @submit="saveForm" class="q-gutter-md">
      <q-input label="Ocorrência" v-model="otherEvent" dense></q-input>
      <q-input v-model="otherDate" for="other-date" label="Data que Ocorreu" type="date" class="q-my-md" mask="date" :rules="['date']" id="other-date" dense></q-input>
      <q-card-actions align="right">
        <q-btn flat color="red-5" label="Cancelar" @click="cancelForm"></q-btn>
        <q-btn flat color="secondary" label="Salvar" type="submit"></q-btn>
      </q-card-actions>
    </q-form>
  </div>
</template>

<script>
import { ref } from "vue";

export default {
  setup() {
    const otherEvent = ref("");
    const otherDate = ref(null);

    function saveForm() {
      const formData = {
        otherEvent: otherEvent.value,
        otherDate: otherDate.value,
      };

      // Aqui você pode fazer uma requisição ao backend para salvar os dados
      console.log(formData);
    }

    function cancelForm() {
      // Lógica para cancelar o formulário, se necessário
    }

    return {
      otherEvent,
      otherDate,
      saveForm,
      cancelForm,
    };
  },
};
</script>
